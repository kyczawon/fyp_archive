with apps as (
    ...
    WHERE name in ('com.microsoft.emmx', 'com.brave.browser', 'com.android.chrome', 'com.opera.browser', 'org.mozilla.firefox') and battery_state='Discharging' limit 1000
),
t_start as (
    select *
from (
...
select t1.device_id as device_id,
            ...
            avg(current_now) as current_now,
            importance,
...
with apps as (
    SELECT samples.id as id, importance, battery_details.voltage as voltage, current_now, devices.capacity as capacity, name, samples.device_id as device_id, battery_level, app_processes.created_at as d, model
    FROM app_processes JOIN samples ON app_processes.sample_id = samples.id JOIN devices ON samples.device_id = devices.id JOIN battery_details ON battery_details.sample_id = samples.id 
    WHERE name in ('com.microsoft.emmx', 'com.brave.browser', 'com.android.chrome', 'com.opera.browser', 'org.mozilla.firefox') and battery_state='Discharging'  and importance='Foreground app' limit 1000000
),
t_start as (
    select *
from (
...
select
    ...
    sum(total_discharge) / sum(TimeDiff) * capacity * 3.6 / sum(TimeDiff) * AVG(avg_voltage) as avg_power,
    -- AVG(power) as avg_power,
    AVG(power2) as avg_power2
    ...
    FROM (
        select t1.device_id as device_id,
            ...
            name,
             -- P = VI, P = V*C(columbs)/t= V*3.6*C(mAh)/t= V*3.6*(t)/t
            -- (MAX(battery_level) - min(battery_level)) * capacity * 3.6 / (TIME_TO_SEC(TIMEDIFF(t_boundaries.t2,t_boundaries.t1))) * AVG(voltage) as power,
            AVG(voltage) * AVG(current_now) / -1000 as power2,
            avg(voltage) as avg_voltage,
            -- avg(current_now) as avg_current,
            importance,
