...
Insert ignore into result SELECT model, capacity, name,num_samples,avg_discharge from (
    with apps as (
        (SELECT samples.id as id, devices.voltage as voltage, devices.capacity as capacity, name, samples.device_id as device_id, battery_level, app_processes.created_at as d, model
...
select
    ...
    sum(total_discharge) / sum(DaysDiff) * capacity * 3.6 / sum(DaysDiff) * AVG(voltage) as avg_discharge
FROM (
        select model,
        count,
