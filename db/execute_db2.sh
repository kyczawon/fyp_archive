...
for name in "${arr[@]}"
do
    ARRAY+=("(SELECT samples.id as id, voltage, devices.capacity as capacity, name, samples.device_id as device_id, battery_level, app_processes.created_at as d, model FROM app_processes JOIN samples ON app_processes.sample_id = samples.id JOIN devices ON samples.device_id = devices.id JOIN battery_details ON battery_details.sample_id = samples.id WHERE name = '$name' and importance='Foreground app' and battery_state='Discharging' and devices.capacity is NOT NULL and samples.created_at > '2018-01-01' limit $limit)")
